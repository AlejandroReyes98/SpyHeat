...
def ImprimeDatos(ax,bx,ay,by,Nx,Ny,hx,hy,A,cd1,B,cd2,C,cd3,D,cd4):
    ...
    print("| Cond. de front. ", cd1, "en BOTTOM : A = %g" % A)
    print("| Cond. de front. ", cd2, "en TOP    : B = %g" % B)
    print("| Cond. de front. ", cd3, "en LEFT   : C = %g" % C)
    print("| Cond. de front. ", cd4, "en RIGHT  : D = %g" % D)
    print("+----------------------------------------------------+")
...
def GraficaSuperficieC(xg,yg,u,colormap):
    f1 = plt.figure()    
    c= plt.contourf(xg, yg, u, 8, alpha=.75, cmap=colormap)
    f1.gca().set_aspect('equal')
    f1.colorbar(c, shrink=1.0)

    f2 = plt.figure()
    ax = f2.gca(projection='3d')    
    s = ax.plot_surface(xg, yg, u, cmap=colormap)
